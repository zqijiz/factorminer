...
def cummax_np(x: np.ndarray) -> np.ndarray:
    # Vectorized cumulative max using accumulate to avoid slow Python loops while handling NaNs correctly
    return np.fmax.accumulate(x, axis=1)


def cummin_np(x: np.ndarray) -> np.ndarray:
    # Vectorized cumulative min using accumulate to avoid slow Python loops while handling NaNs correctly
