...
def kama_np(x: np.ndarray, window: int = 10) -> np.ndarray:
    ...
    # Bolt Optimization: Pre-calculate rolling volatility to avoid in-loop nansum
    diff_abs = np.abs(np.diff(x, axis=1))
    diff_abs = np.nan_to_num(diff_abs, nan=0.0)
    diff_abs_pad = np.concatenate([np.zeros((M, 1), dtype=np.float64), diff_abs], axis=1)
    cs = np.cumsum(diff_abs_pad, axis=1)
    ...
    for t in range(window, T):
        ...
        # Bolt Optimization: Avoid boolean slice indexing with branchless where
...
def ema_torch(x: torch.Tensor, window: int = 10) -> torch.Tensor:
    ...
    for t in range(1, T):
        ...
        # Bolt Optimization: Replace boolean slice assignment with branchless where
        # to avoid triggering CPU-GPU synchronizations on advanced indexing
        new_val = alpha * curr + (1 - alpha) * prev
        out[:, t] = torch.where(both, new_val, torch.where(only_prev, prev, out[:, t]))
    return out
...
def kama_torch(x: torch.Tensor, window: int = 10) -> torch.Tensor:
    ...
    # Bolt Optimization: Pre-calculate rolling volatility to avoid in-loop operations
    diff_abs = x.diff(dim=1).abs()
    diff_abs = torch.nan_to_num(diff_abs, nan=0.0)
    diff_abs_pad = torch.cat([torch.zeros((M, 1), dtype=x.dtype, device=x.device), diff_abs], dim=1)
    cs = diff_abs_pad.cumsum(dim=1)

    for t in range(window, T):
        direction = (x[:, t] - x[:, t - window]).abs()
        vol = cs[:, t] - cs[:, t - window]
