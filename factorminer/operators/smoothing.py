...
def kama_np(x: np.ndarray, window: int = 10) -> np.ndarray:
    ...
    diffs[:, 1:] = np.abs(np.diff(x, axis=1))
    diffs = np.nan_to_num(diffs)

    diff_cs = np.concatenate([np.zeros((M, 1)), np.cumsum(diffs, axis=1)], axis=1)

    for t in range(window, T):
        dir_t = direction[:, t]
        vol_t = diff_cs[:, t + 1] - diff_cs[:, t - window + 1]

        # Safe divide avoiding RuntimeWarning and intermediate array
        er = np.divide(dir_t, vol_t, out=np.zeros_like(dir_t), where=vol_t > 1e-10)
...
def kama_torch(x: torch.Tensor, window: int = 10) -> torch.Tensor:
    ...
    diffs[:, 1:] = x.diff(dim=1).abs()
    diffs = torch.nan_to_num(diffs)

    diff_cs = torch.cat(
        [torch.zeros(M, 1, dtype=x.dtype, device=x.device), diffs.cumsum(dim=1)], dim=1
    )
    ...
    for t in range(window, T):
        ...
        er = torch.where(vol_t > 1e-10, dir_t / vol_t, torch.zeros_like(dir_t))
        sc = (er * (fast_sc - slow_sc) + slow_sc) ** 2
