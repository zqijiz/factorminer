...
def kama_np(x: np.ndarray, window: int = 10) -> np.ndarray:
    ...
    # Pre-calculate rolling volatility to avoid O(T*window) inner loop overhead
    ...
    if T - 1 >= window:
        vol_all = np.sum(sliding_window_view(abs_diff, window, axis=1), axis=2)
    else:
        vol_all = np.zeros((M, max(0, T - window)), dtype=np.float64)

    for t in range(window, T):
        direction = np.abs(x[:, t] - x[:, t - window])
        volatility = vol_all[:, t - window]
        ...
        # Branchless update to avoid array slice assignment overhead
        calc = prev + sc * (curr - prev)
        out[:, t] = np.where(valid, calc, out[:, t])
    return out
...
def ema_torch(x: torch.Tensor, window: int = 10) -> torch.Tensor:
    ...
    for t in range(1, T):
        ...
        # Branchless update to avoid slice indexing causing kernel launches
        calc = alpha * curr + (1 - alpha) * prev
        out[:, t] = torch.where(both, calc, torch.where(only_prev, prev, out[:, t]))
    return out
...
def kama_torch(x: torch.Tensor, window: int = 10) -> torch.Tensor:
    ...
    # Pre-calculate rolling volatility to avoid inner loop overhead
    abs_diff = x.diff(dim=1).abs().nan_to_num(0.0)

    if T - 1 >= window:
        vol_all = abs_diff.unfold(1, window, 1).sum(dim=2)
    else:
        vol_all = torch.zeros((M, max(0, T - window)), device=x.device, dtype=x.dtype)
    ...
    for t in range(window, T):
        ...
        # Branchless update to avoid slice indexing causing kernel launches
        calc = prev + sc * (curr - prev)
        out[:, t] = torch.where(valid, calc, out[:, t])
    return out
